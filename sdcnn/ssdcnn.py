...
# Dilated CNN (234) with Shared and Separable Convolution (SSC) included
def ss_dcnn(patch_height, patch_width, n_ch):
    print('********************SS_DCNN********************')
...
def dilation_block(_inputs, _filters, _kernel_size, _strides=1, _dilation_rate=1, ssc=False):
    ...
    if ssc == True:
        block_1 = SeparableConv2D(_filters, _kernel_size, activation='relu', padding='same', data_format='channels_last', dilation_rate=_dilation_rate)(block_1)
    else:
